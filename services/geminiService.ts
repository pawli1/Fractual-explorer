...
export const getStrategicHint = async (
  ...
  try {
    ...
    const isQuotaError = errorMsg.includes("429") || errorMsg.toLowerCase().includes("quota") || errorMsg.toLowerCase().includes("exhausted");
    
    return {
        hint: getBestLocalTarget(isQuotaError ? "AI Quota Exhausted" : "AI Service Unreachable"),
        debug: { ...debug, error: errorMsg, isQuotaExceeded: isQuotaError }
    };
  }
};
