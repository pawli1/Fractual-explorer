...
import { Loader2, Trophy, BrainCircuit, Play, MousePointerClick, Eye, Terminal, Clock, AlertTriangle, Target, Lightbulb, Monitor, Key, ExternalLink } from 'lucide-react';
...
const GeminiSlingshot: React.FC = () => {
  ...
  const performAiAnalysis = async (screenshot: string) => {
    ...
    const aiResponse = await getStrategicHint(screenshot, allClusters, maxRow);
    ...
    if (debug.isQuotaExceeded) {
        setQuotaExceeded(true);
        setAiHint("Strategy engine paused: Quota exhausted.");
    } else {
  ...
  const handleSelectKey = async () => {
    try {
        ...
        setQuotaExceeded(false);
        // Retry capture after key selection
        captureRequestRef.current = true;
    } catch (e) {
  ...
  const drawHandAndSlingshot = (ctx: CanvasRenderingContext2D, anchorX: number, anchorY: number, canvasHeight: number) => {
    ctx.save();
    const woodGradient = ctx.createLinearGradient(anchorX - 50, anchorY, anchorX + 50, anchorY + 100);
    ...
    ctx.beginPath();
    ctx.lineWidth = 14;
    ctx.moveTo(anchorX, anchorY + 40);
    ctx.lineTo(anchorX, canvasHeight);
    ctx.stroke();

    ctx.beginPath();
    ctx.lineWidth = 16;
    ctx.moveTo(anchorX - 45, anchorY - 15);
    ctx.quadraticCurveTo(anchorX - 45, anchorY + 40, anchorX, anchorY + 40);
    ctx.quadraticCurveTo(anchorX + 45, anchorY + 40, anchorX + 45, anchorY - 15);
    ctx.stroke();

    ctx.fillStyle = '#3e2723';
    ctx.beginPath();
    ctx.arc(anchorX - 45, anchorY - 15, 10, 0, Math.PI * 2);
    ctx.arc(anchorX + 45, anchorY - 15, 10, 0, Math.PI * 2);
    ctx.fill();

    const gloveColor = '#263238'; 
    const accentColor = '#42a5f5';

    ctx.shadowBlur = 0;
    ctx.fillStyle = gloveColor;
    ctx.beginPath();
    ctx.roundRect(anchorX - 22, anchorY + 45, 44, 60, 10);
    ctx.fill();
    ...
    for (let i = 0; i < 4; i++) {
        ctx.beginPath();
        const fy = anchorY + 50 + i * 14;
        ctx.roundRect(anchorX - 25, fy, 50, 12, 5);
        ctx.fill();
        ctx.stroke();
    }

    ctx.beginPath();
    ctx.ellipse(anchorX + 15, anchorY + 65, 10, 18, Math.PI / 4, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(anchorX - 10, anchorY + 55);
    ctx.lineTo(anchorX + 10, anchorY + 55);
    ctx.stroke();
    ctx.restore();
  };

  const drawPouch = (ctx: CanvasRenderingContext2D, ballX: number, ballY: number, angle: number) => {
    ctx.save();
    ctx.translate(ballX, ballY);
    ctx.rotate(angle);
    const pouchGrad = ctx.createLinearGradient(-15, -15, 15, 15);
    pouchGrad.addColorStop(0, '#424242');
    pouchGrad.addColorStop(1, '#212121');
    ctx.fillStyle = pouchGrad;
    ctx.beginPath();
    ctx.moveTo(-15, -BUBBLE_RADIUS - 2);
    ctx.quadraticCurveTo(-25, 0, -15, BUBBLE_RADIUS + 2);
    ctx.lineTo(10, BUBBLE_RADIUS + 2);
    ctx.quadraticCurveTo(0, 0, 10, -BUBBLE_RADIUS - 2);
    ctx.closePath();
    ctx.fill();
    ctx.restore();
  };
  ...
  useEffect(() => {
    ...
    const onResults = (results: any) => {
            ...
            if (collisionOccurred) {
                ...
                for (let r = 0; r < GRID_ROWS + 5; r++) {
                    ...
                    for (let c = 0; c < colsInRow; c++) {
                        ...
                        if (dist < bestDist) {
                            bestDist = dist; bestRow = r; bestCol = c; bestX = x; bestY = y;
                        }
                    }
                }
      ...
      const bandColor = isPinching.current ? '#ffee58' : 'rgba(255, 238, 88, 0.6)';
      if (!isFlying.current) {
        ctx.save();
        ctx.beginPath();
      ...
      if (!isFlying.current) {
          ...
          drawPouch(ctx, ballPos.current.x, ballPos.current.y, angle);
      }
      ...
      drawHandAndSlingshot(ctx, anchorPos.current.x, anchorPos.current.y, canvas.height);
  ...
  return (
    <div className="flex w-full h-screen bg-[#121212] overflow-hidden font-roboto text-[#e3e3e3]">
      ...
      <div className="fixed inset-0 z-[100] bg-[#121212] flex flex-col items-center justify-center p-8 text-center md:hidden">
         ...
         <p className="text-[#c4c7c5] max-w-md text-lg leading-relaxed">Please maximize window on a larger screen.</p>
      </div>
      ...
      <div ref={gameContainerRef} className="flex-1 relative h-full overflow-hidden">
        ...
        {quotaExceeded && (
            <div className="absolute top-24 left-1/2 -translate-x-1/2 z-[60] animate-in fade-in slide-in-from-top-4 duration-300">
                <div className="bg-[#1e1e1e] border-2 border-[#ef5350] rounded-2xl p-4 shadow-2xl flex flex-col items-center gap-3 max-w-sm text-center">
                    <div className="bg-[#ef5350]/20 p-2 rounded-full">
                        <AlertTriangle className="w-6 h-6 text-[#ef5350]" />
                    </div>
                    <div>
                        <h3 className="text-sm font-bold text-white uppercase tracking-tight">API Quota Exhausted</h3>
                        <p className="text-xs text-[#c4c7c5] mt-1 leading-snug">The strategy engine is resting. Use your personal API key for uninterrupted gameplay.</p>
                    </div>
                    <button 
                        onClick={handleSelectKey}
                        className="w-full bg-[#ef5350] hover:bg-[#d32f2f] text-white py-2 px-4 rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-2"
                    >
                    ...
                    <a 
                        ...
                        className="text-[10px] text-[#42a5f5] hover:underline flex items-center gap-1"
                    >
            ...
            <div className="bg-[#1e1e1e] px-6 py-4 rounded-[32px] border border-[#444746] shadow-2xl flex items-center gap-4">
                ...
                {availableColors.length === 0 ? <p className="text-sm text-gray-500">No ammo</p> : (
                    COLOR_KEYS.filter(c => availableColors.includes(c)).map(color => {
                        const isSelected = selectedColor === color, isRecommended = aiRecommendedColor === color, config = COLOR_CONFIG[color];
                        return (
        ...
        <div className="p-5 border-b-4 transition-colors duration-500 flex flex-col gap-2" style={{ backgroundColor: '#252525', borderColor: borderColor }}>
             ...
             {aiRecommendedColor && (
                <div className="flex items-center gap-2 mt-3 bg-black/20 p-2 rounded">
                    <Target className="w-4 h-4 text-gray-400" /><span className="text-xs text-gray-400 uppercase tracking-wide">Rec. Color:</span>
             ...
             {quotaExceeded && (
                 <button onClick={handleSelectKey} className="mt-2 text-[10px] text-[#ef5350] hover:underline flex items-center gap-1 font-bold">
                    <Key className="w-3 h-3" /> Upgrade to personal API key
                 </button>
             )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
            <div>
                ...
                <div className={`p-3 rounded-lg border ${isAiThinking ? 'bg-[#a8c7fa]/10 border-[#a8c7fa]/30 text-[#a8c7fa]' : 'bg-[#444746]/20 border-[#444746]/50 text-[#c4c7c5]'}`}>
                    <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${isAiThinking ? 'bg-[#a8c7fa] animate-pulse' : (quotaExceeded ? 'bg-[#ef5350]' : 'bg-[#66bb6a]')}`} />
                        <span className="text-sm font-mono">{isAiThinking ? 'Processing Vision...' : (quotaExceeded ? 'Quota Limited' : 'Waiting for Input')}</span>
                    </div>
                </div>
            </div>
            {debugInfo?.screenshotBase64 && (
                <div>
                    <div className="flex items-center gap-2 mb-2 text-[#c4c7c5] text-xs font-bold uppercase tracking-wider"><Eye className="w-3 h-3" /> Vision Input</div>
            ...
            {debugInfo && (
                <div>
                    <div className="flex items-center gap-2 mb-2 text-[#c4c7c5] text-xs font-bold uppercase tracking-wider"><BrainCircuit className="w-3 h-3" /> AI Output</div>
                    <div className="grid grid-cols-2 gap-2 mb-3">
                         ...
                         <div className="bg-[#2a2a2a] p-2 rounded border border-[#444746]"><p className="text-[10px] text-gray-500 mb-1">Status</p><div className={`text-[10px] font-mono font-bold ${quotaExceeded ? 'text-[#ef5350]' : 'text-[#66bb6a]'}`}>{quotaExceeded ? '429 Quota' : '200 OK'}</div></div>
                    </div>
                    {debugInfo.error && (
                         <div className={`p-3 rounded-lg mb-3 border ${quotaExceeded ? 'bg-[#ef5350]/10 border-[#ef5350]/30 text-[#ef5350]' : 'bg-[#1e1e1e] border-[#444746] text-gray-400'}`}>
                            <div className="flex items-start gap-2">
                                <AlertTriangle className="w-4 h-4 mt-0.5 shrink-0" />
                                <div><p className="text-xs font-bold uppercase tracking-tight">API Message</p><p className="text-[10px] font-mono mt-1 break-all leading-relaxed">{debugInfo.error}</p></div>
                            </div>
                         </div>
                    )}
                </div>
